...
| In-hand atlas (phone) | At both rounds, every rat, ~30 s. Film from directly above with the lid open, or the rat on a plain towel - not through the acrylic wall (glare, reflections, blur); no coloured tape bits in the background (they are the coban colours); hold still ~2 s per view: top-down with the whole body in frame, left side, right side, head-on; same distance (~30 cm) and light every time; one still of the back from above. | none | 30 clips: the human reference for every marker and for the dark/white patch pattern (the body-wide cue that also shows under IR), the tape state on the last days. Reference material, not training data for the box camera (different camera, light, viewpoint, background). |
